...
# Constants
FIRSTBOOT_MARKER = '/opt/ontoportal/config/firstboot'.freeze
CONFIG_FILE = '/opt/ontoportal/config/site_config.rb'.freeze
...
# Setup logging to stdout only (journald will capture it)
...
# Begin script
begin
  # Ensure the script is run by the 'op-admin' user
  if ENV['USER'] != 'op-admin' && Process.uid != Etc.getpwnam('op-admin').uid
    $logger.fatal "Script must be run as user 'ontoportal'. Current user: #{ENV['USER']}"
    exit 1
  end

  unless File.file?(FIRSTBOOT_MARKER)
    $logger.info 'Script skipped: not the first boot.'
    exit
  end

  $logger.info 'Starting firstboot script...'

  # Wait for OntoPortal services to be up
  n = 0
  until system('sudo /usr/local/bin/opctl status')
    $logger.info 'Some services are not running. Waiting 15 seconds...'
    sleep 15
    n += 1
    raise 'OntoPortal services failed to start.' if n > 5
  end
  ...
  def aws_metadata_instance_id
    ...
    begin
      ...
      $logger.warn "AWS metadata query failed: #{e.message}"
    end
  ...
  if instance_id
    ...
    begin
      ...
      if response.code.to_i.between?(200, 299)
        $logger.info "Admin password set to instance ID: #{instance_id}"
      else
        $logger.error "Failed to set admin password: #{response.code} #{response.body}"
      end
    rescue StandardError => e
      $logger.error "Error setting admin password: #{e.message}"
    end
  end
  ...
  FileUtils.chown 'op-admin', 'op-ui', UI_CONFIG_DIR
